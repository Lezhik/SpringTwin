...
| [x] F1.5 | Создать базовый layout и навигацию | Medium |
| [x] F1.6 | Настроить сборку Vue через Gradle | Medium |
...
- [x] `gradlew.bat build` успешно выполняется
- [x] `gradlew.bat test` успешно выполняется
- [x] Приложение запускается на localhost:8080
- [x] Vue.js приложение открывается в браузере
