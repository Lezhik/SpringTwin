...
- Тесты: JUnit 5 для unit и integration тестов на backend, Playwright для E2E тестов (запускаются из Spring Boot тестов), Vitest для unit тестов на frontend
...
### Сборка и тестирование UI

Если нужно отдельно от проекта собрать и/или протестировать UI, то нужно в папке src/main/vue выполнить команды:

сборка: npm run build

выполнение тестов: set CI=true && npm run test

при сборке фронта через gradle - gradle будет так же вызывать эти команды для сборки фронта
...
| **architecture** | Архитектурные сущности | Модели графа: ClassNode, MethodNode, EndpointNode, FieldNode, связи между ними (DEPENDS_ON, CALLS, INSTANTIATES, ACCESSES_FIELD, EXPOSES_ENDPOINT) |
...
└── test/
    ...
    │   └── twin/
    │       └── spring/
    │           ├── profiles/              # Тестовые профили (общие для всех уровней)
    │           │   ├── app/
    │           │   ├── project/
    │           │   ├── architecture/
    │           │   ├── analysis/
    ...
    │           │   ├── PlaywrightConfiguration.java
    │           │   └── TestNeo4jConfig.java
    │           ├── unit/                  # Level 1: Unit тесты
    │           │   ├── app/
    │           │   ├── project/
    │           │   ├── architecture/
    │           │   ├── analysis/
    ...
    │           │   ├── migration/
    │           │   └── mcp/
    │           ├── integration/           # Level 2: Integration тесты
    │           │   ├── app/
    │           │   ├── project/
    │           │   ├── architecture/
    │           │   ├── analysis/
    ...
    │           │   ├── migration/
    │           │   └── mcp/
    │           └── e2e/                   # Level 3: E2E тесты (Playwright)
    │               ├── app/
    │               ├── project/
    │               ├── architecture/
    │               ├── analysis/
    │               ├── report/
    │               └── mcp/
    └── vue/                               # Unit тесты frontend (Vitest)
        ├── app/
        ├── project/
        ├── architecture/
        ├── analysis/
...
|------|----------|----------|
| **ClassNode** | Java класс | name, fullName, packageName, labels |
| **MethodNode** | Метод класса | name, signature, returnType, modifiers |
| **EndpointNode** | REST endpoint | path, httpMethod, produces, consumes |
| **FieldNode** | Поле класса | name, type, modifiers |

### Метки узлов (Labels)

Для узлов ClassNode используются метки Neo4j, формируемые на основе Spring аннотаций:
...
| Связь | От | К | Описание |
|-------|-----|---|----------|
| **DEPENDS_ON** | ClassNode | ClassNode | Dependency Injection связь |
| **CALLS** | MethodNode | MethodNode | Вызов метода |
| **INSTANTIATES** | MethodNode | ClassNode | Создание экземпляра класса |
| **ACCESSES_FIELD** | MethodNode | FieldNode | Доступ к полю |
| **EXPOSES_ENDPOINT** | MethodNode | EndpointNode | Метод暴露 REST endpoint |
...
graph LR
    ...
    MethodNode -->|INSTANTIATES| ClassNode
    MethodNode -->|EXPOSES_ENDPOINT| EndpointNode
    MethodNode -->|ACCESSES_FIELD| FieldNode
```
...
### Уровни тестирования Backend

Проект использует трехуровневую структуру тестов на backend:

| Уровень | Расположение | Покрытие | Инструменты |
|---------|--------------|----------|-------------|
| **Unit** | `src/test/java/twin/spring/unit/` | Сервисы, мапперы, модели | JUnit 5 + Mockito |
| **Integration** | `src/test/java/twin/spring/integration/` | REST API, репозитории | Spring Boot Test + Neo4j Embedded |
| **E2E** | `src/test/java/twin/spring/e2e/` | Пользовательские сценарии | Playwright + Spring Boot Test |

### Unit тесты

- Тестируют отдельные компоненты в изоляции
- Используют Mockito для мокирования зависимостей
- Не требуют Spring контекста
- Быстрое выполнение

### Integration тесты

- Тестируют REST API и репозитории
- Используют Spring Boot Test контекст
- Используют Neo4j embedded (in-memory)
- WebTestClient для тестирования API

### E2E тесты (Playwright)

- Тестируют полные пользовательские сценарии через UI
- Запускаются из Spring Boot тестов с Playwright browser
- Используют Neo4j embedded (in-memory)
- Реальный HTTP сервер

### Frontend тесты

- Только unit тесты с использованием Vitest
- Компоненты, store, сервисы
- Запускаются из директории `src/main/vue`

### Тестовые профили

- Используй тестовые профили для заполнения данных, вместо прямого заполнения данных в тестах
- Профили расположены в `src/test/java/twin/spring/profiles/`
- В качестве профилей выступают специальные классы, заполняющие соответствующие модели и DTO
- Профили используются всеми уровнями тестов (unit, integration, e2e)

### Семантические селекторы

Используй атрибуты `data-test`, `name` и `role` для селекторов в E2E тестах:
...
- Все методы контроллеров должны покрываться integration тестами
...
- Всегда запускай выполнение тестов после выполнения задачи

### CLI команды для тестов
...
- Применение принципов SOLID при разработке
- Используй Pinia для управления состоянием UI
- Имена классов, полей, методов и переменных должны соответствовать правилам семантической разметки, код должен быть самодокументируемым
...
- при сборке и запуске тестов корневой папкой является src/main/vue, а не корень проекта
- код хранится строго по модулям: src/main/vue/<module> и src/test/vue/<module>
- модули находятся напрямую в src/main/vue/<module>/ без промежуточной папки src/
- корневые файлы приложения (App.vue, main.ts, vite-env.d.ts) находятся в src/main/vue/
...
- Отдельные файлы для тестов по уровням (unit, integration, e2e) - содержат специфические правила для каждого уровня тестирования
...
#### Файлы тестирования бэкенд (по уровням)
- src/test/java/twin/spring/unit/AGENTS.md - правила для unit тестов
- src/test/java/twin/spring/unit/<module>/AGENTS.md - правила для unit тестов модуля
- src/test/java/twin/spring/integration/AGENTS.md - правила для integration тестов
- src/test/java/twin/spring/integration/<module>/AGENTS.md - правила для integration тестов модуля
- src/test/java/twin/spring/e2e/AGENTS.md - правила для E2E тестов
- src/test/java/twin/spring/e2e/<module>/AGENTS.md - правила для E2E тестов модуля

#### Файлы тестирования фронтенд
- src/test/vue/AGENTS.md - правила для тестирования фронтенд
- src/test/vue/<module>/AGENTS.md - правила для тестирования модуля фронтенд
