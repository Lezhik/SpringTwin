...
|----|----------|------|---------------------|
| IT-F1.2-01 | Запуск всех тестов | 1. Выполнить `npm run test` | Все тесты проходят |
| IT-F1.2-02 | Генерация coverage | 1. Выполнить `npm run test:coverage` | Отчет создан |
...
- [ ] Добавлена зависимость vitest в package.json
- [ ] Добавлена зависимость @vue/test-utils
- [ ] Создан vitest.config.ts в src/main/vue
- [ ] Настроены скрипты в package.json: test, test:coverage, test:watch
- [ ] Создан пример теста для Vue компонента
- [ ] Создан пример теста для TypeScript функции
- [ ] Настроена генерация coverage отчета
- [ ] Настроен junit xml reporter для CI
- [ ] Команда `npm run test` выполняется успешно
- [ ] Команда `npm run test:coverage` генерирует отчет
