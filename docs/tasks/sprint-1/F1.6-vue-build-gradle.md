...
2. Автоматически копировать собранные файлы в build/resources/main/public
...
graph TD
    ...
    subgraph Output
        resources_public[build/resources/main/public]
        js_files[js/]
        css_files[css/]
        assets[assets/]
    end
...
|----|----------|------|---------------------|
| IT-F1.6-01 | Полная сборка через Gradle | 1. Выполнить `gradlew.bat build` | Файлы в build/resources/main/public |
...
- [x] Добавлен plugin com.github.node-gradle.node в build.gradle.kts
- [x] Настроена задача npmInstall для установки зависимостей
- [x] Настроена задача npmBuild для сборки Vue
- [x] Настроена задача npmTest для запуска тестов
- [x] Создана задача vueBuild агрегирующая npm install и build
- [x] Создана задача vueClean для очистки
- [x] Задача build зависит от vueBuild
- [x] Задача test зависит от npmTest
- [x] Output директория настроена в build/resources/main/public
- [x] Команда `gradlew.bat build` выполняет полную сборку
- [x] Команда `gradlew.bat test` выполняет все тесты
- [x] Собранные файлы доступны через Spring Boot
