...
Анализирует Java исходники и извлекает Spring DI зависимости.

## Команда

    spring-twin scan-source
      --project <path>
      --output <file>
      --include <mask>
      --exclude <mask>
...
## Параметры
  ...
  параметр      описание
  ------------- -------------------------------
  `--project`   путь к исходному коду проекта
  `--output`    путь к JSON результату
  `--include`   маска пакетов для анализа
  `--exclude`   маска пакетов для исключения
...
## Параметры
  ...
  `--output`    путь к JSON результату
  `--include`   маска пакетов
  `--exclude`   маска пакетов
