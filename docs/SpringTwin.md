...
- projectPath (путь к исходному коду проекта для сканирования)
- includePackages
- excludePackages
...
Для узлов графа (в первую очередь ClassNode) используются метки (labels) Neo4j.
...
## Backend
- JUnit 5 — unit тесты
- Spring Boot Test — integration тесты

## Frontend
- Vitest — unit тесты

## E2E
- Playwright — end-to-end тестирование веб-интерфейса (запускается из Spring Boot тестов)
